...
-- Create table for PWA events tracking
CREATE TABLE public.pwa_events (
  id uuid NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY,
  event_type text NOT NULL CHECK (event_type IN ('impression', 'install', 'dismiss_temporary', 'dismiss_permanent')),
...
CREATE POLICY "Admins can view pwa events"
ON public.pwa_events
FOR SELECT
USING (public.has_role(auth.uid(), 'admin'));

-- Create index for faster queries
CREATE INDEX idx_pwa_events_type ON public.pwa_events(event_type);
CREATE INDEX idx_pwa_events_created ON public.pwa_events(created_at DESC);

-- Update get_admin_app_pulse to include PWA stats
...
BEGIN
  ...
  -- Star families (top 10 by completion rate in last 7 days)
