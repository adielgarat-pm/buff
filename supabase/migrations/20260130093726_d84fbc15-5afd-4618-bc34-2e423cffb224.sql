...
BEGIN
  ...
  -- Star families with per-child stats (top 10 by completion count)
  ...
  FROM (
    SELECT jsonb_build_object(
      ...
      'children', (
        SELECT COALESCE(jsonb_agg(
          jsonb_build_object(
            ...
            'completion_rate', CASE 
              WHEN (SELECT COUNT(*) FROM daily_progress dp WHERE dp.child_id = pc.id AND (dp.date::date) >= (CURRENT_DATE - INTERVAL '7 days')::date) > 0 
              THEN ROUND(
                ((SELECT COUNT(*) FROM daily_progress dp WHERE dp.child_id = pc.id AND dp.completed = true AND (dp.date::date) >= (CURRENT_DATE - INTERVAL '7 days')::date)::NUMERIC / 
                 (SELECT COUNT(*) FROM daily_progress dp WHERE dp.child_id = pc.id AND (dp.date::date) >= (CURRENT_DATE - INTERVAL '7 days')::date)) * 100
              )
              ELSE 0 
            END
        ...
        FROM profiles pc
        WHERE pc.family_id = f.id AND pc.role = 'child'
      ),
