...
DROP FUNCTION IF EXISTS public.get_admin_app_pulse();

CREATE FUNCTION public.get_admin_app_pulse()
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  AND dp.date >= (CURRENT_DATE - INTERVAL '7 days')::date;
  ...
  FROM daily_progress
  WHERE completed = true AND date >= (CURRENT_DATE - INTERVAL '7 days')::date;
  
  -- Potential completions (tasks * active children for the day)
  ...
  -- Logins in last 24h (estimate from recent activity)
  v_logins_24h := 0;
  ...
  FROM (
    ...
    FROM daily_progress dp
    WHERE dp.date >= (CURRENT_DATE - INTERVAL '28 days')::date
    GROUP BY date_trunc('week', dp.date)
  ) week_data;
  
  -- Star families (top 10 by completion count) - fixed subquery
  SELECT COALESCE(jsonb_agg(
    jsonb_build_object(
      ...
      'recent_rewards', COALESCE(
        (SELECT jsonb_agg(jsonb_build_object('title', sr.title, 'claimed_at', sr.claimed_at))
         FROM (
           ...
           FROM store_rewards 
           WHERE family_id = star_data.family_id AND claimed = true
  ...
  FROM (
    SELECT 
      ...
      COUNT(DISTINCT dp.id) FILTER (WHERE dp.completed = true) as completion_count,
      CASE 
        WHEN COUNT(DISTINCT dp.id) > 0 
        THEN ROUND((COUNT(DISTINCT dp.id) FILTER (WHERE dp.completed = true)::NUMERIC / COUNT(DISTINCT dp.id)) * 100)
        ELSE 0 
      END as completion_rate
    FROM families f
    LEFT JOIN profiles p ON p.family_id = f.id AND p.role = 'child'
    LEFT JOIN daily_progress dp ON dp.family_id = f.id AND dp.date >= (CURRENT_DATE - INTERVAL '7 days')::date
    GROUP BY f.id, f.name, f.short_code
    ORDER BY COUNT(DISTINCT dp.id) FILTER (WHERE dp.completed = true) DESC
    LIMIT 10
  ) star_data;
  ...
  SELECT jsonb_build_object(
    ...
    'lesson_completions_7d', (SELECT COUNT(*) FROM lesson_progress WHERE completed = true AND date >= (CURRENT_DATE - INTERVAL '7 days')::date),
