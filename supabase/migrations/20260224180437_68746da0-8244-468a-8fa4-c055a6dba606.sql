...
BEGIN
  ...
  -- Low engagement - filtered by date range
  ...
  -- Shared vs separate device children
  ...
  FROM (
    ...
    FROM daily_progress dp
    WHERE (dp.date::date) >= v_start_date
  ...
  FROM (
    SELECT 
      ...
      (SELECT json_agg(json_build_object('title', sr.title, 'claimed_at', sr.claimed_at))
       ...
       ORDER BY sr.claimed_at DESC LIMIT 5) as recent_rewards
    FROM families f
    CROSS JOIN LATERAL (
      SELECT 
        COUNT(*) FILTER (WHERE dp.completed = true) as comp_count,
        COUNT(*) as pot_count
    ...
    WHERE EXISTS (SELECT 1 FROM profiles p WHERE p.family_id = f.id AND p.role = 'child')
      AND comp_count > 0
    ORDER BY completion_rate DESC, comp_count DESC
    LIMIT 10
  ) sf;
