...
BEGIN
  ...
  WHERE (f.created_at::date) >= v_start_date AND (f.created_at::date) <= v_end_date
    AND EXISTS (
      SELECT 1 FROM daily_progress dp JOIN profiles p ON dp.child_id = p.id
      WHERE p.family_id = f.id AND dp.completed = true AND dp.completed_at < (NOW() - INTERVAL '4 days')
    )
    AND NOT EXISTS (
      SELECT 1 FROM daily_progress dp2 JOIN profiles p2 ON dp2.child_id = p2.id
      WHERE p2.family_id = f.id AND dp2.completed = true AND dp2.completed_at >= (NOW() - INTERVAL '4 days')
    )
    AND (NOT p_exclude_test_accounts OR NOT EXISTS (
  ...
  FROM (
    ...
    LEFT JOIN (
      SELECT (dp.completed_at::date) as activity_date, COUNT(DISTINCT p.family_id) as active_families
      FROM daily_progress dp JOIN profiles p ON dp.child_id = p.id
      WHERE dp.completed = true AND dp.completed_at IS NOT NULL
      GROUP BY (dp.completed_at::date)
  ...
  WHERE dp.completed = true AND dp.completed_at >= (NOW() - INTERVAL '7 days');
  ...
  -- STAR FAMILIES (now includes preferred_language)
  SELECT json_agg(sf ORDER BY sf.completion_rate DESC) INTO v_star_families
  FROM (
    SELECT 
      f.id as family_id, f.name as family_name, f.short_code as family_code,
      ...
      (SELECT json_agg(child_info) FROM (
        SELECT cp.id as child_id, cp.display_name,
    ...
    WHERE (f.created_at::date) >= v_start_date AND (f.created_at::date) <= v_end_date
    GROUP BY f.id, f.name, f.short_code, f.preferred_language
