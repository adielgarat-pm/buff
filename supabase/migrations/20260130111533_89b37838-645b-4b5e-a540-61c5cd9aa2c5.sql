-- Drop and recreate get_admin_app_pulse with active users today fix
...
BEGIN
  ...
  FROM (
    SELECT dp.child_id as active_child
    FROM daily_progress dp
    WHERE dp.completed = true 
      AND dp.child_id IS NOT NULL
      AND dp.date = to_char(CURRENT_DATE, 'YYYY-MM-DD')
    UNION
    SELECT lp.child_id as active_child
    FROM lesson_progress lp
    WHERE lp.completed = true 
      AND lp.child_id IS NOT NULL
      AND lp.date = to_char(CURRENT_DATE, 'YYYY-MM-DD')
  ) active_users;
