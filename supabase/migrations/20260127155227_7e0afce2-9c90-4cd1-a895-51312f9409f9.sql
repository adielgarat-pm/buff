-- Fix the default task creation trigger to use proper HH:MM time format
-- instead of text categories like 'morning', 'afternoon', 'evening'
...
BEGIN
  ...
  IF NEW.role = 'child' AND NEW.family_id IS NOT NULL THEN
    ...
    -- SCHOOL DAY TEMPLATE (Morning tasks - 06:00-09:00)
    INSERT INTO public.tasks (family_id, assigned_to, title, category, time, credits, icon, description)
    VALUES 
      (NEW.family_id, NEW.id, 'התלבשות ונעליים', 'hygiene', '07:00', 15, '👕', 'להתלבש ולהתארגן לבוקר'),
      (NEW.family_id, NEW.id, 'ארוחת בוקר ותרופות', 'nutrition', '07:30', 10, '🍳', 'לאכול ארוחת בוקר ולקחת תרופות'),
      (NEW.family_id, NEW.id, 'בדיקת תיק', 'school', '08:00', 10, '🎒', 'לוודא שכל הציוד בתיק');
    
    -- SCHOOL DAY TEMPLATE (Afternoon tasks - 16:00-20:00)
    INSERT INTO public.tasks (family_id, assigned_to, title, category, time, credits, icon, description)
    VALUES 
      (NEW.family_id, NEW.id, 'פריקת תיק', 'school', '16:00', 15, '📚', 'לפרוק את התיק ולהוציא מסמכים'),
      (NEW.family_id, NEW.id, 'התחלת שיעורי בית', 'school', '17:00', 20, '✏️', 'להתחיל בשיעורי הבית');
    
    -- SCHOOL DAY TEMPLATE (Evening tasks - 20:00-24:00)
    INSERT INTO public.tasks (family_id, assigned_to, title, category, time, credits, icon, description)
    VALUES 
      (NEW.family_id, NEW.id, 'הכנה למחר', 'school', '20:00', 15, '📋', 'להכין בגדים ותיק למחר'),
      (NEW.family_id, NEW.id, 'כיבוי מסכים שעה לפני השינה', 'hygiene', '20:30', 25, '📵', 'לכבות טלפון וטלוויזיה'),
      (NEW.family_id, NEW.id, 'מקלחת וצחצוח שיניים', 'hygiene', '21:00', 15, '🚿', 'להתקלח ולצחצח שיניים');
