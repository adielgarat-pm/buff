CREATE OR REPLACE FUNCTION public.get_admin_app_pulse_v2(p_start_date date DEFAULT NULL::date, p_end_date date DEFAULT NULL::date, p_exclude_test_accounts boolean DEFAULT false)
 RETURNS json
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  -- RED FLAGS: Stuck onboarding now shows exact step
  ...
  FROM (
    SELECT d.date,
      ...
      (SELECT COUNT(DISTINCT p.family_id) FROM daily_progress dp JOIN profiles p ON dp.child_id = p.id WHERE dp.completed = true AND (dp.date::date) = d.date) as active_families
    FROM generate_series(v_start_date, v_end_date, '1 day'::interval) d(date)
  ) trend_data;

  SELECT json_agg(cat_data) INTO v_category_completions
  FROM (
    SELECT t.category, COUNT(*) FILTER (WHERE dp.completed = true) as completions, COUNT(*) as potential
    FROM daily_progress dp JOIN tasks t ON dp.task_id = t.id
  ...
  SELECT COUNT(DISTINCT daily_progress.child_id) INTO v_active_children_7d FROM daily_progress WHERE daily_progress.completed = true AND daily_progress.child_id IS NOT NULL AND (daily_progress.date::date) >= (CURRENT_DATE - INTERVAL '7 days')::date;
  ...
  FROM (
    SELECT daily_progress.child_id as active_child FROM daily_progress WHERE daily_progress.completed = true AND daily_progress.child_id IS NOT NULL AND daily_progress.date = to_char(CURRENT_DATE, 'YYYY-MM-DD')
    UNION
    SELECT lesson_progress.child_id as active_child FROM lesson_progress WHERE lesson_progress.completed = true AND lesson_progress.child_id IS NOT NULL AND lesson_progress.date = to_char(CURRENT_DATE, 'YYYY-MM-DD')
  ) active_users;
  ...
  SELECT ARRAY_AGG(DISTINCT u.email) INTO v_marketing_emails FROM profiles p JOIN auth.users u ON p.user_id = u.id WHERE p.marketing_consent = true AND u.email IS NOT NULL;
  ...
  FROM (
    SELECT u.id as user_id, u.email, u.created_at, u.last_sign_in_at,
      ...
      p.role as profile_role, p.display_name,
  ...
  FROM (
    ...
    FROM daily_progress WHERE (date::date) >= (CURRENT_DATE - INTERVAL '8 weeks')::date
  ...
  FROM (
    SELECT f.id as family_id, f.name as family_name, f.short_code as family_code,
      ...
      (SELECT json_agg(child_stats) FROM (
        SELECT p.id as child_id, p.display_name,
          COUNT(*) FILTER (WHERE dp.completed = true) as completion_count, COUNT(*) as potential_count,
          CASE WHEN COUNT(*) > 0 THEN ROUND((COUNT(*) FILTER (WHERE dp.completed = true)::numeric / COUNT(*)::numeric) * 100) ELSE 0 END as completion_rate
        FROM profiles p LEFT JOIN daily_progress dp ON dp.child_id = p.id AND (dp.date::date) >= (CURRENT_DATE - INTERVAL '7 days')::date
        WHERE p.family_id = f.id AND p.role = 'child' GROUP BY p.id, p.display_name
      ) child_stats) as children,
      (SELECT COUNT(*) FROM profiles WHERE family_id = f.id AND role = 'child') as child_count,
      COALESCE((SELECT COUNT(*) FROM daily_progress dp JOIN profiles p ON dp.child_id = p.id WHERE p.family_id = f.id AND dp.completed = true AND (dp.date::date) >= (CURRENT_DATE - INTERVAL '7 days')::date), 0) as completion_count,
      CASE WHEN (SELECT COUNT(*) FROM daily_progress dp JOIN profiles p ON dp.child_id = p.id WHERE p.family_id = f.id AND (dp.date::date) >= (CURRENT_DATE - INTERVAL '7 days')::date) > 0 
        THEN ROUND((SELECT COUNT(*) FROM daily_progress dp JOIN profiles p ON dp.child_id = p.id WHERE p.family_id = f.id AND dp.completed = true AND (dp.date::date) >= (CURRENT_DATE - INTERVAL '7 days')::date)::numeric / (SELECT COUNT(*) FROM daily_progress dp JOIN profiles p ON dp.child_id = p.id WHERE p.family_id = f.id AND (dp.date::date) >= (CURRENT_DATE - INTERVAL '7 days')::date)::numeric * 100)
        ELSE 0 END as completion_rate,
      (SELECT json_agg(reward_data ORDER BY reward_data.claimed_at DESC) FROM (SELECT sr.title, sr.claimed_at FROM store_rewards sr WHERE sr.family_id = f.id AND sr.claimed = true ORDER BY sr.claimed_at DESC LIMIT 5) reward_data) as recent_rewards
    FROM families f
    WHERE EXISTS (SELECT 1 FROM profiles WHERE family_id = f.id AND role = 'child')
    ORDER BY (SELECT COUNT(*) FROM daily_progress dp JOIN profiles p ON dp.child_id = p.id WHERE p.family_id = f.id AND dp.completed = true AND (dp.date::date) >= (CURRENT_DATE - INTERVAL '7 days')::date) DESC
    LIMIT 10
  ) family_data;
  ...
  SELECT json_build_object(
    ...
    'by_os', (SELECT json_agg(json_build_object('os', os, 'count', cnt)) FROM (SELECT os, COUNT(*) as cnt FROM pwa_events WHERE event_type = 'install' AND os IS NOT NULL GROUP BY os) os_stats),
    'by_browser', (SELECT json_agg(json_build_object('browser', browser, 'count', cnt)) FROM (SELECT browser, COUNT(*) as cnt FROM pwa_events WHERE event_type = 'install' AND browser IS NOT NULL GROUP BY browser) browser_stats)
  ) INTO v_pwa_stats;
...
-- Also update get_admin_profiles_overview to include onboarding_step and is_activated
