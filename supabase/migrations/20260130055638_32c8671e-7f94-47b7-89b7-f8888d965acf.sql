-- Fix admin analytics function: cast text date columns to DATE to avoid operator errors
...
BEGIN
  ...
  -- Active children in last 7 days (daily_progress.date is TEXT)
  ...
  -- Star families (top 10 by completion count)
  SELECT COALESCE(jsonb_agg(
    jsonb_build_object(
  ...
  FROM (
    ...
    LEFT JOIN daily_progress dp ON dp.family_id = f.id AND (dp.date::date) >= (CURRENT_DATE - INTERVAL '7 days')::date
    GROUP BY f.id, f.name, f.short_code
    ORDER BY COUNT(DISTINCT dp.id) FILTER (WHERE dp.completed = true) DESC
    LIMIT 10
  ) star_data;

  -- School Quest stats (lesson_progress.date is TEXT)
