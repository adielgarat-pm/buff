...
BEGIN
  ...
  -- Star families with parent email, children names, and marketing consent (top 10 by completion count)
  ...
  FROM (
    SELECT jsonb_build_object(
      ...
      'children_names', (
        SELECT COALESCE(jsonb_agg(pc.display_name ORDER BY pc.created_at), '[]'::jsonb)
        FROM profiles pc
