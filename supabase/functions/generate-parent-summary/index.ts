...
serve(async (req) => {
  ...
  try {
    ...
    const prompt = `אתה יועץ חינוכי ידידותי שעוזר להורים. צור סיכום שבועי קצר ומעודד בעברית עבור הורה, בהתבסס על הנתונים הבאים:
...
⚡ **אתגרים לתשומת לב:**
[תאר את האתגרים בצורה רכה ולא שיפוטית]

💡 **טיפ להמשך:**
[תן עצה אחת מעשית וקונקרטית. אם האתגר קשור לשיעורי בית או לימודים, הצע לפרק למשימות קטנות עם טיימר. אם קשור לשגרת בוקר, הצע להכין ציוד בערב דרך School Quest]
